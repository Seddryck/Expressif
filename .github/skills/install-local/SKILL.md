...
The install directory is mandatory. By default, it must be empty or not yet exist. If the user explicitly wants to reuse a non-empty directory, pass `-Force`; files from the archive can then overwrite files with the same names, while unrelated existing files remain in place. The script downloads exactly one `expressif-*-net10.0-win-x64.zip`, extracts it into that directory, and verifies `expressif.exe`. It uses a unique temporary download directory and removes that directory afterward.
