...
internal static class ConfiguredOutput
{
    public static bool TryResolve(CliConfiguration configuration, string command, ValueFormat? requested,
        ...
        => TryResolveStyle(configuration, command, requested, pretty, compact, indent,
    ...
    public static bool TryResolve(CliConfiguration configuration, string command,
        ...
        out ValueFormattingOptions formatting, out string? error)
    {
        formatting = new ValueFormattingOptions();
        if (!OutputSerializationSelection.TryResolve(output, raw, out serializer, out error))
            return false;

        if (!TryResolveStyle(configuration, command, requested, pretty, compact, indent,
                out var style, out var indentation, out error))
        {
            return false;
        }
        ...
        {
            var width = int.Parse(
                configuration.Get(command + ".preferred-line-width"),
                System.Globalization.CultureInfo.InvariantCulture);
            var inlineTypes = configuration.Get(command + ".inline-types");
            formatting = CliValueFormatting.Create(style, indentation, width, inlineTypes);
            return true;
        }
