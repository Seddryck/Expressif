...
internal sealed class RunHandler
{
    ...
    public RunHandler(CliServices services)
    {
    ...
    public int Execute(RunRequest request)
    {
            ...
            var sequenceInput = request.HasSource
                ? BuildSourceRows(request.SourcePath, request.SourceOptions, request.Scalar)
                : BuildInputSequence(request.InputRows, request.HasBatch, request.BatchInput);

            var context = new Context();
            IExpression openExpression;
            try
            {
                openExpression = services.Expressions.CompileOpen(expressionCode, context);
            }
            ...
            {
                return ExpressionCommandCommon.WriteValidationError(exception, expressionCode, hasExpressionFile, request.ExpressionFilePath);
            }
            ...
            try
            {
                foreach (var result in EvaluateEach(openExpression, context, sequenceInput))
    ...
    private IEnumerable<object?> BuildInputSequence(
        ...
        string? batchInput)
    {
        foreach (var input in inputRows)
        {
            ...
            try
            {
                parsedInput = services.Values.Parse(input);
            }
            catch (FormatException exception)
            {
        ...
        try
        {
            parsedBatchInput = services.Values.Parse(batchInput ?? string.Empty);
        }
        catch (FormatException exception)
        {
    ...
    internal IEnumerable<object?> BuildSourceRows(string? sourcePath, IReadOnlyList<string> sourceOptions, bool scalar = false)
    {
