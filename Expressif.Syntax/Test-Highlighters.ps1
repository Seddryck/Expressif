...
if ('~' -notin $operators -or ':>' -notin $operators -or '-' -in $operators -or '@' -in $operators) {
    throw 'UDL operators must include tuple and input binding markers without splitting variable references or hyphenated names.'
}
...
$variablePrefixes = ($keywords | Where-Object name -eq 'Keywords5').'#text' -split ' '
if ('@' -notin $variablePrefixes) { throw 'UDL must recognize named variable references by prefix.' }
