...
internal static class DocumentationExtensions
{
    ...
    /// </summary>
    /// <param name="type"></param>
    /// <param name="typeMapper">Maps runtime types to Expressif type names.</param>
    /// <param name="options">Provides vocabulary-specific parameter metadata.</param>
    /// <returns></returns>
    internal static CtorInfo[] GetInfoConstructors(
        this Type type,
        ExpressifTypeMapper typeMapper,
        IntrospectionOptions options)
    {
        ...
        foreach (var ctorNode in ctorNodes)
        {
            ...
            {
                var key = new ParameterIntrospectionKey(type, names[i]);
                paramInfos.Add(new ParamInfo(
                    options.ParameterNames.GetValueOrDefault(key, names[i]),
                    typeMapper.ToExpressifType(
