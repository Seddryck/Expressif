...
public class FunctionIntrospectorTest
{
    ...
    public void Describe_UnconvertedFunctions_AreExplicitlyReported()
        => Assert.That(
            ...
            Is.EquivalentTo(new[] { "apply", "catch", "coalesce", "conditional-forward", "field", "guard", "let", "nested-field", "neutral", "recode", "switch", "throw", "try", "walk", "with" }));
    ...
    [TestCase("flat-map", "expression", "expression")]
    ...
    [Test]
    public void Locate_ExpressifAssembly_ArrayFunctionsExposeAccumulatorsWrappersAndShifts()
    {
        using (Assert.EnterMultipleScope())
        {
