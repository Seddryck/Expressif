...
public class OpenArgumentTest
{
    ...
    {
        var expression = TestExpression.CreateClosed(code);
        var literal = TestExpression.CreateClosed(code.Replace("(.reference)", "(\"invalid\")", StringComparison.Ordinal));
    ...
    public void Evaluate_NonBooleanCondition_StillRejectsResult(string code)
        => Assert.That(() => TestExpression.CreateClosed(code).Evaluate(null),
