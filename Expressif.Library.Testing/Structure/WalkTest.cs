...
public class WalkTest
{
    ...
    public void Evaluate_Record_TransformsValuesAndPreservesFieldNames()
    {
        ...
        var result = (RecordValue)TestExpression.Create("walk(*trim)").Evaluate(input)!;
    ...
    public void Evaluate_NestedRecords_TransformsLeavesAndPreservesEveryFieldName()
    {
        ...
        var result = (RecordValue)TestExpression.Create("walk(*trim)").Evaluate(customer)!;
    ...
    public void Evaluate_GuardedPipeline_AppliesWholeExpressionOnlyToCompatibleLeaves()
        => Assert.That(TestExpression.Create("walk(*(trim | append-space))").Evaluate(new TupleValue(42, " Bob ")),
