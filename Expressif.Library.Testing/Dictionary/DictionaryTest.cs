...
public class DictionaryTest
{
    ...
    public void DuplicateStructuralKeys_ThrowsExplicitly()
        => Assert.That(
            () => TestExpression.Create("dictionary(({1, 2} => \"first\"), ({1, 2} => \"second\"))").Evaluate(null),
    ...
    public void OrdinaryTupleAndRecord_AreRejected()
    {
        Assert.Multiple(() =>
        {
            Assert.That(() => TestExpression.Create("dictionary(T(1, 2))").Evaluate(null), Throws.ArgumentException);
            Assert.That(() => TestExpression.Create("dictionary({key := 1, value := 2})").Evaluate(null), Throws.ArgumentException);
        });
    }
}
