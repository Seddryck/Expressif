...
public class CubeTest
{
    ...
    public void Evaluate_RejectsInconsistentDimensions(string expression)
        => Assert.That(() => TestExpression.Create(expression).Evaluate(null), Throws.ArgumentException.With.Message.StartsWith("Grouping keys must be all scalar"));
    ...
    public void Evaluate_RejectsAlreadyExpandedKeys(string expression)
        => Assert.That(() => TestExpression.Create(expression).Evaluate(null), Throws.ArgumentException.With.Message.StartsWith("Grouping keys must not contain an aggregated dimension"));
