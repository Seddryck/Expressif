...
public class GroupingTest
{
    ...
    public void Literal_RoundTripsGroupingAndGroupRuntimeTypes()
    {
        ...
        var parsed = TestExpression.CreateClosed(ValueFormatter.Format(source)).Evaluate(null);
    ...
    public void Group_IsTransparentToArrayFunctions()
        => Assert.That(
            TestExpression.CreateClosed("#{(\"BE\" => {\"alice\", \"bob\"}), (\"FR\" => {\"charlie\"})} | map(cardinality)").Evaluate(null),
