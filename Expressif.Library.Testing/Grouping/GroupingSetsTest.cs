...
public class GroupingSetsTest
{
    ...
    public void Evaluate_RejectsInvalidSetsAndKeys(string expression)
        => Assert.That(() => TestExpression.Create(expression).Evaluate(null), Throws.InstanceOf<ArgumentException>());

    [Test]
    public void Bind_RejectsNamedArguments()
        => Assert.That(() => TestExpression.Create("#{} | grouping-sets(values := tuple())"), Throws.InstanceOf<Expressif.Bindings.BindingException>());
    ...
    public void Evaluate_MatchesSpecializedExpansion(string specialized, string sets)
    {
        ...
        Assert.That(TestExpression.Create($"{source} | grouping-sets({sets})").Evaluate(null),
            Is.EqualTo(TestExpression.Create($"{source} | {specialized}").Evaluate(null)));
    }
