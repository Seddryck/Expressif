...
public static class ConformanceTestCaseDataSource
{
    ...
    private static string ResolveExpectedFile(string conformanceRoot, string fullName, string testName)
    {
        ...
        if (namespaceSegments.Length == 0)
            throw new InvalidOperationException($"Namespace '{relativeNamespace}' must identify a scope.");

        var fileToken = testName.Split('_', StringSplitOptions.RemoveEmptyEntries).FirstOrDefault() ?? testName;
        var scope = ToKebabCase(namespaceSegments[0]);
        var fileName = ToKebabCase(fileToken) + ".yaml";
        var candidates = Directory.EnumerateFiles(conformanceRoot, fileName, SearchOption.AllDirectories)
            .Where(path => string.Equals(
                Directory.GetParent(path)?.Name,
                scope,
                StringComparison.OrdinalIgnoreCase))
            .Where(path => Deserialize(path)?.Tests?.Any(test => ToPascalSnakeCase(test.Id) == testName) == true)
            .ToArray();

        if (candidates.Length == 0)
        {
            throw new FileNotFoundException(
                $"Conformance YAML file '{fileName}' was not found for scope '{scope}', type '{fullName}', and test '{testName}'.");
        }
        if (candidates.Length > 1)
        {
            throw new InvalidOperationException(
                $"Multiple conformance YAML files match scope '{scope}' and test '{testName}': {string.Join(", ", candidates)}.");
        }

        var expectedFile = candidates[0];
    ...
    private static object NormalizeArray(object normalized, Type targetType)
    {
        ...
        var array = System.Array.CreateInstance(elementType, rawItems.Length);
    ...
    private static string FormatArrayDisplay(System.Array array)
        => array.Length == 0
            ? "{ }"
