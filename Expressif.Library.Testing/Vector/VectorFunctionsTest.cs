...
public class VectorFunctionsTest
{
    ...
    public void Vector_InvalidSpreadComponent_Throws()
        => Assert.That(
            () => TestExpression.CreateClosed("V(...T(1, \"x\"))").Evaluate(null),
    ...
    public void TupleOperation_VectorExpression_PreservesVector(string source, string expected)
        => Assert.That(ValueFormatter.Format(TestExpression.CreateClosed(source).Evaluate(null)), Is.EqualTo(expected));

    [Test]
    public void Extend_VectorWithNonnumericComponent_ReturnsNull()
        => Assert.That(TestExpression.CreateClosed("V(1, 2) | extend(\"x\")").Evaluate(null), Is.Null);
}
