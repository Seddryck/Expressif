...
public class DistanceTest
{
    ...
    public void Expression_VectorDistance_ReturnsExpected()
        => Assert.That(TestExpression.CreateClosed("V(1, 2) | distance(V(4, 6))").Evaluate(null), Is.EqualTo(5m));

    [Test]
    public void Expression_DimensionMismatch_ReturnsNull()
        => Assert.That(TestExpression.CreateClosed("V(1, 2) | distance(V(3, 4, 5))").Evaluate(null), Is.Null);
