...
public class GenerateTest
{
    ...
    public void Generate_Valid_While_Next_OptionalResult(object? input, string[] parameters, decimal[] expected)
    {
        ...
        var result = parameters.Length == 2
            ? new ExpressionFactory(new ExpressionBinder()).Create($"generate(while := {parameters[0]}, next := {parameters[1]})").Evaluate(seed)
            : new ExpressionFactory(new ExpressionBinder()).Create($"generate(while := {parameters[0]}, next := {parameters[1]}, result := {parameters[2]})").Evaluate(seed);
    ...
    {
        var generate = new ExpressionFactory(new ExpressionBinder()).Create(
    ...
    private static object? Evaluate(object input, string[] parameters)
        => new ExpressionFactory(new ExpressionBinder()).Create(
