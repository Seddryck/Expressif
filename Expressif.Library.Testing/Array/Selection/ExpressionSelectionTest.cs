...
public class ExpressionSelectionTest
{
    ...
    {
        var result = TestExpression.Create(expression).Evaluate(input);
    ...
    {
        var expression = TestExpression.Create("{{id := 1, bar := 10}, {id := 2, bar := 30}, {id := 3, bar := 30}, {id := 4, bar := 50}} | " + function + " | .id");
    ...
    public void Evaluate_TextCriteria_UsesOrdinalOrder(string name, string expected)
        => Assert.That(TestExpression.Create(name + "(neutral)").Evaluate(new[] { "z", "a" }), Is.EqualTo(expected));
    ...
    {
        var result = TestExpression.Create(expression).Evaluate(new object[] { 2, 3m, 1.0 });
    ...
    public void Evaluate_InvalidCriteria_Fail(string expression)
        => Assert.That(() => TestExpression.Create(expression).Evaluate(new[] { new object() }), Throws.ArgumentException);
    ...
    {
        var info = ExpressifIntrospection.Functions.Describe().Single(function => function.Name == name);
        Assert.Multiple(() =>
        {
    ...
    {
        var expression = TestExpression.Create("max-by(absolute)");
    ...
    public void ClosestBy_EquidistantCriteria_PreservesFirst()
        => Assert.That(TestExpression.Create("closest-by(neutral, 3)").Evaluate(new[] { 4, 2 }), Is.EqualTo(4));

    [Test]
    public void ClosestBy_TextCriterion_IsRejected()
        => Assert.That(() => TestExpression.Create("closest-by(neutral, 3)").Evaluate(new[] { "3" }), Throws.ArgumentException);

    [TestCase("min-by(neutral)")]
    [TestCase("max-by(neutral)")]
    public void Evaluate_IncompatibleCriteria_Fail(string expression)
        => Assert.That(() => TestExpression.Create(expression).Evaluate(new object[] { 1, "a" }), Throws.ArgumentException);

    [Test]
    public void Evaluate_DateCriteria_PreserveOriginalValue()
    {
        var first = new DateOnly(2026, 1, 1);
        Assert.That(TestExpression.Create("min-by(neutral)").Evaluate(new[] { first.AddDays(1), first }), Is.EqualTo(first));
    }
