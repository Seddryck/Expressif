...
public class ChunkWhileTest
{
    ...
    public void ChunkWhile_ExplicitBinding(object? value, string expression, string expected)
        => Assert.That(TestExpression.Create(expression).Evaluate(value is string source ? new ParameterValueConverter().Parse(source) : value), Is.EqualTo(TestExpression.Create(expected).Evaluate(null)));
    ...
    public void ChunkWhile_Valid_Operation(object? input, string operation, string? expected)
    {
        ...
        var actual = TestExpression.Create($"chunk-while({operation})").Evaluate(value);
    ...
    public void Expression_NumericGaps_FormsConsecutiveChunks()
        => Assert.That(
            TestExpression.Create("chunk-while($1 | subtract($0 | last) | is-less-than(2))")
    ...
    public void Evaluate_NonBooleanOperation_ReturnsNull()
        => Assert.That(TestExpression.Create("chunk-while($0 | cardinality)").Evaluate(new[] { 1, 2 }), Is.Null);
    ...
    {
        var expression = TestExpression.Create("chunk-while($0 | cardinality | less-than(2))");
