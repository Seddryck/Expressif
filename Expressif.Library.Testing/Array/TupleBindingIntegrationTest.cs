...
public class TupleBindingIntegrationTest
{
    ...
    public void ExplicitBinding_Composes(string source, string expected)
        => Assert.That(TestExpression.Create(source).Evaluate(null), Is.EqualTo(TestExpression.Create(expected).Evaluate(null)));
    ...
    public void CanonicalForms_KeepFollowingArgumentContexts(string map, string shorthand, string canonical)
    {
        ...
        Assert.That(TestExpression.Create(map + "(" + shorthand + suffix).Evaluate(5),
            Is.EqualTo(TestExpression.Create(map + "(" + canonical + suffix).Evaluate(5)));
    }
    ...
    public void SplitFollowingArguments_RetainEnclosingContext()
        => Assert.That(TestExpression.Create("{text := \"aaabb\", flag := #true} | .text | split-while(starts-with~ | is-equal-to(.flag))")
            .Evaluate(null), Is.EqualTo(new[] { "aaa", "bb" }));

    private static object? EvaluateCustom(string source, object? input)
    {
        var factory = new FunctionFactory(
            new CompositeImplementationRegistry(
                new ImplementationRegistry(
                [
                    new("counted", typeof(Counted)),
