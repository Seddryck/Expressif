...
public class FilterTest
{
    ...
    public void Filter_Valid_OpenArguments(object? value, string code, string expected)
        => Assert.That(TestExpression.CreateClosed(code).Evaluate(value),
            Is.EqualTo(TestExpression.CreateClosed(expected).Evaluate(null)));
    ...
    public void Evaluate_AndPredicate_Valid()
        => Assert.That(new Filter(() => TestPredication.Create("greater-than(2) |AND less-than(5)")).Evaluate(new object[] { 1, 2, 3, 4, 5 }), Is.EqualTo(new object?[] { 3, 4 }));
