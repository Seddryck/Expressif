...
public class TupleScopeTest
{
    ...
    {
        var input = TestExpression.CreateClosed(source).Evaluate(null);
    ...
    public void Parse_QuotedReference_RemainsLiteral()
        => Assert.That(TestExpression.CreateClosed("\"^^$1\"").Evaluate(null), Is.EqualTo("^^$1"));
    ...
    public void Evaluate_Failure_RestoresEnclosingFrame()
    {
        ...
        var expression = TestExpression.Create("apply(T(1, 2) | tuple(...#null))");
    ...
    public void Bind_ScopedCoercionSelector_DoesNotDiscardScope()
        => Assert.Throws<BindingException>(() => TestExpression.Create("coerce(^$0 -> :numeric)"));
    ...
    {
        var expression = TestExpression.Create("apply(^$0 | add(^^$1))");
