...
public class LabelTest
{
    ...
    public void Label_DuplicateResultingField_Throws()
        => Assert.That(
            () => TestExpression.CreateClosed("T(1, 2) | label(\"same\", \"same\")").Evaluate(null),
    ...
    public void LabelConflicts_ScalarLabelConflictingWithRecordField_QualifiesRecordField()
        => Assert.That(
            ValueFormatter.Format(TestExpression.CreateClosed("T(42, {id := 1}) | label-conflicts(\"id\", \"record\")").Evaluate(null)),
            Is.EqualTo("{id := 42, \"record.id\" := 1}"));

    [Test]
    public void Label_ArgumentsUseEnclosingContext()
        => Assert.That(
            ValueFormatter.Format(TestExpression.CreateClosed(
    ...
    public void Label_SpreadArgumentsAreRejected(string function)
        => Assert.That(
            () => TestExpression.CreateClosed($"T(1, 2) | {function}(...{{\"a\", \"b\"}})").Evaluate(null),
    ...
    private static void AssertResult(string function, string value, string[] names, string expected)
    {
        ...
        var result = TestExpression.Create($"{value} | {call}").Evaluate(null);
