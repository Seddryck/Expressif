...
public class TupleFunctionsTest
{
    ...
    private static IPositionalValue ParseTupleLike(string value)
        => value.Contains("=>", StringComparison.Ordinal)
            ? (IPositionalValue)TestExpression.CreateClosed(value).Evaluate(null)!
            : ParseTuple(value);
    ...
    public void Evaluate_FromEndProjection_ReturnsExpected(string expression, object? expected)
        => Assert.That(TestExpression.CreateClosed(expression).Evaluate(null), Is.EqualTo(expected));
    ...
    public void Extend_Valid(string value, object? extension, string expected)
    {
        ...
        Assert.That(
            TestExpression.CreateClosed($"{NormalizeTupleSyntax(value)} | extend({parameter})").Evaluate(null),
            Is.EqualTo(ParseTupleLike(expected)));
    }
    ...
    public void Extend_EvaluatesSupportedParameterShapes(string source, string expected)
        => Assert.That(TestExpression.CreateClosed(source).Evaluate(null), Is.EqualTo(ParseTuple(expected)));
    ...
    public void Pick_Valid(string value, int[] positions, string expected)
        => Assert.That(
            TestExpression.Create($"pick({string.Join(", ", positions)})").Evaluate(ParseTupleLike(value)),
            Is.EqualTo(ParseTupleLike(expected)));
