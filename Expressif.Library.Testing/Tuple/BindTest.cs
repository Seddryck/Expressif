...
public class BindTest
{
    ...
    public void Bind_Invoke(string value, string function, string expected)
        => Assert.That(TestExpression.Create($"bind(\"{function}\")").Evaluate(TestExpression.Create(value).Evaluate(null)),
    ...
    public void Shorthand_Subtraction(string source, int expected)
        => Assert.That(TestExpression.Create(source).Evaluate(null), Is.EqualTo(expected));
    ...
    public void Nested_ExtendUsesItsTuple(string source)
        => Assert.That(TestExpression.Create(source).Evaluate(null), Is.EqualTo(new TupleValue(120m, 135m, 15m)));
    ...
    {
        var error = Assert.Throws<TupleBindingException>(() => TestExpression.Create(source).Evaluate(null));
    ...
    {
        var error = Assert.Throws<TupleBindingException>(() => TestExpression.Create("missing~"));
    ...
    {
        var expression = TestExpression.Create("subtract~");
    ...
    public void NameExpression_KeepsEnclosingContext()
        => Assert.That(TestExpression.Create("{target := \"subtract\", values := T(9, 4)} | .values | bind(.target)").Evaluate(null), Is.EqualTo(5));
    ...
    public void InputBinding_RetainsDiagnosticOffsets()
    {
        ...
        var error = Assert.Throws<TupleBindingException>(() => TestExpression.Create(source));
