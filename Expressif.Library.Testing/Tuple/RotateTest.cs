...
public class RotateTest
{
    ...
    private static object? ParseTuple(string value)
        => TestExpression.Create(value.Replace("T(", "tuple(", StringComparison.Ordinal)).Evaluate(null);
    ...
    public void Offset_BindingUsesEnclosingContext(string source)
        => Assert.That(TestExpression.CreateClosed(source).Evaluate(null), Is.EqualTo(new TupleValue(30m, 10m, 20m)));
    ...
    public void Evaluate_NonPositionalInput_ReturnsNull(object? input)
        => Assert.That(TestExpression.Create("rotate").Evaluate(input), Is.Null);
    ...
    public void InvalidOffset_UsesStandardIntegerValidation(string operation)
        => Assert.That(() => TestExpression.Create(operation).Evaluate(new TupleValue(10, 20, 30)), Throws.Exception);

    [Test]
    public void NullOffset_UsesStandardZeroConversion()
        => Assert.That(TestExpression.Create("rotate(#null)").Evaluate(new TupleValue(10, 20, 30)),
            Is.EqualTo(new TupleValue(10, 20, 30)));

    [Test]
    public void Introspection_ReportsTupleContractAndOptionalInteger()
    {
        var info = ExpressifIntrospection.Functions.Describe().Single(info => info.Name == "rotate");
        Assert.Multiple(() =>
        {
    ...
    {
        var expression = TestExpression.Create("$1 | rotate($0)");
