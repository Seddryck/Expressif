...
public class NestedFieldTest
{
    ...
    public void NamedArgument_IsRejectedDuringBinding()
        => Assert.That(
            () => TestExpression.Create("nested-field(path := \"name\")"),
    ...
    {
        var expression = TestExpression.Create("nested-field(.path, \"value\")");
        Parallel.For(0, 20, i =>
        {
            var input = new RecordValue();
    ...
    {
        var info = ExpressifIntrospection.Functions.Describe().Single(info => info.Name == "nested-field");
