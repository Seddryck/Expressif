...
public class VisibilityFunctionsTest
{
    ...
    public void Visibility_InvalidNames_ThrowsClearError(string source)
        => Assert.That(() => TestExpression.Create(source).Evaluate(new RecordValue()),
    ...
    public void Visibility_Collision_DoesNotMutateInput(string name)
    {
        ...
        input.Set("_id", 42);
        input.Set("id", "ABC");
        Assert.That(() => TestExpression.Create(name).Evaluate(input), Throws.TypeOf<InvalidOperationException>());
