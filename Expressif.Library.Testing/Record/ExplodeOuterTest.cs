...
public class ExplodeOuterTest
{
    ...
    public void NonCollectionField_FailsEvaluation(string selected)
        => Assert.That(() => TestExpression.Create($"{{tags := {selected}}} | explode-outer(.tags)").Evaluate(null),
    ...
    public void NonRecordParent_FailsEvaluation(string source)
        => Assert.That(() => TestExpression.Create($"{source} | explode-outer(.tags)").Evaluate(null),
            Throws.TypeOf<ArgumentException>());
    ...
    {
        var expression = TestExpression.Create("explode-outer(.tags)");
        Parallel.For(0, 20, i =>
        {
    ...
    {
        var info = ExpressifIntrospection.Functions.Describe().Single(info => info.Name == "explode-outer");
        using (Assert.EnterMultipleScope())
        {
