...
public class RecordFunctionsTest
{
    ...
    public void FieldSelection_NonTextName_ThrowsClearError(string source)
        => Assert.That(
            () => TestExpression.Create(source).Evaluate(new ValueRecord()),
    ...
    public void PutPath_InvalidPath_ThrowsClearError(string expression, string message)
        => Assert.That(
            () => TestExpression.Create(expression).Evaluate(null),
