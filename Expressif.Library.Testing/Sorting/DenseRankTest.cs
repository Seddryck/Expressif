...
public class DenseRankTest
{
    [Conformance]
    public void DenseRank_Valid_Rows(object? input, string expression, string expected)
        => Assert.That(ValueFormatter.Format(TestExpression.Create(expression).Evaluate(input)), Is.EqualTo(expected));

    [Test]
    public void DenseRank_InvalidNonNullComparison_Throws()
    {
        var expression = TestExpression.Create("array((sort-key(sort-term(\"bad\", compare-numeric~)) => 1), (sort-key(sort-term(2, compare-numeric~)) => 2)) | sort-table | dense-rank");
        Assert.That(() => expression.Evaluate(null), Throws.TypeOf<InvalidOperationException>());
    }

    [Test]
    public void DenseRank_InvalidRowArity_Throws()
    {
        var table = new SortTableValue([], [new SortRow([1], "invalid")]);
        Assert.That(() => new DenseRank().Evaluate(table), Throws.TypeOf<ArgumentException>());
    }

    [Test]
    public void DenseRank_UnsupportedInput_Throws()
        => Assert.That(() => ((IFunction)new DenseRank()).Evaluate(new object()), Throws.TypeOf<ArgumentException>());

    [Test]
    public void DenseRank_HeaderlessRows_AllTieAndPreserveOriginalValues()
    {
    ...
    [Test]
    public void DenseRank_ComparerEqualDistinctRuntimeValues_ShareDenseRank()
    {
        ...
        var result = new DenseRank().Evaluate(table);
        Assert.Multiple(() =>
        {
    ...
    [Test]
    public void DenseRank_PostSortScan_UsesAdjacentOrderedKeys()
    {
        ...
        new DenseRank().Evaluate(table);
        Assert.That(comparisons.TakeLast(3), Is.EqualTo(new (object?, object?)[] { (2, 1), (2, 2), (3, 2) }));
    }

    [Test]
    public void DenseRank_InvalidComparisonDuringTieDetection_Throws()
    {
        var comparisons = 0;
        ...
        Assert.That(() => new DenseRank().Evaluate(table), Throws.TypeOf<InvalidOperationException>()
