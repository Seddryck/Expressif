...
public class BottomWithTiesTest
{
    [Conformance]
    public void BottomWithTies_Valid_Rows(object? input, string expression, string expected)
    ...
    public void NegativeCount_Throws()
        => Assert.That(() => new BottomWithTies(() => -1).Evaluate(new SortTableValue([], [])), Throws.TypeOf<ArgumentOutOfRangeException>());
    ...
    public void InvalidComparer_Throws()
    {
        ...
        Assert.That(() => new BottomWithTies(() => 1).Evaluate(table), Throws.TypeOf<InvalidOperationException>());
    }

    [Test]
    public void ComparerEqualDistinctValues_IncludeEntireBoundary()
    {
        ...
        Assert.That(new BottomWithTies(() => 1).Evaluate(table), Is.EqualTo(new[] { "integer", "decimal" }));
    }
}
