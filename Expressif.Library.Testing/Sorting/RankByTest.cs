...
public class RankByTest
{
    [Conformance]
    public void RankBy_Valid_Criteria(object? input, string expression, string expected)
    ...
    public void ExplicitLowering_MatchesSugar(string criterion, string term)
    {
        ...
        var sugar = TestExpression.Create($"{source} | rank-by({criterion})").Evaluate(null);
        var lowered = TestExpression.Create($"{source} | map(neutral | pair(sort-key({term}), neutral)) | sort-table | rank").Evaluate(null);
        Assert.That(ValueFormatter.Format(sugar), Is.EqualTo(ValueFormatter.Format(lowered)));
    }

    [Test]
    public void MissingCriteria_Throws()
        => Assert.That(() => TestExpression.Create("array() | rank-by()").Evaluate(null), Throws.TypeOf<BindingException>());
}
