...
public class SortTermFunctionsTest
{
    ...
    public void SortTerm_NonComparerReference_Throws()
        => Assert.That(() => TestExpression.Create("sort-term(1, add~)").Evaluate(null), Throws.TypeOf<BindingException>());

    [Test]
    public void SortTerm_ComparerIsNotInvokedDuringConstruction()
        => Assert.That(TestExpression.Create("sort-term(1, compare-numeric~)").Evaluate(null), Is.TypeOf<Expressif.Values.SortTerm>());
