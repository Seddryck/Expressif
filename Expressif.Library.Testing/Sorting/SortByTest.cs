...
public class SortByTest
{
    ...
    public void SortBy_EqualKeys_IsStable()
    {
        ...
        var result = (object?[])TestExpression.Create(source).Evaluate(null)!;
    ...
    public void SortBy_DirectTupleProjection_EvaluatesAgainstEachItem()
    {
        ...
        Assert.That(
            ValueFormatter.Format(TestExpression.Create(source).Evaluate(null)),
            Is.EqualTo("{T(1, 100), T(2, 120), T(3, 110), T(4, 75)}"));
    }

    [Test]
    public void SortBy_MissingCriteria_Throws()
        => Assert.That(() => TestExpression.Create("array() | sort-by()").Evaluate(null), Throws.TypeOf<BindingException>());

    [Test]
    public void SortBy_InvalidCoercion_BecomesNullKey()
        => Assert.That(
            ValueFormatter.Format(TestExpression.Create("array({v := \"bad\"}, {v := 1}) | sort-by(.v -> :numeric)").Evaluate(null)),
            Is.EqualTo("{{v := 1}, {v := \"bad\"}}"));

    [Test]
    public void SortBy_UnsupportedType_ThrowsClearly()
        => Assert.That(
            () => TestExpression.Create("array(1) | sort-by(neutral -> :boolean)").Evaluate(null),
