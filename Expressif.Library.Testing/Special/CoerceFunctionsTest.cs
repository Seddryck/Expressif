...
public class CoerceFunctionsTest
{
    ...
    public void Coerce_Valid_TypeDirected(object? value, string expression, string expected)
    {
        var input = value is string text
            ? text is "#less" or "#equal" or "#greater"
                ? TestExpression.CreateClosed(text).Evaluate(null)
                : text.Contains("=>")
                ? TestExpression.CreateClosed(text).Evaluate(null)
    ...
    public void Coerce_InvalidSpecification_ThrowsBindingError(string expression)
        => Assert.That(() => TestExpression.Create(expression), Throws.Exception);
    ...
    public void Coerce_MissingRecordField_PreservesRecord()
        => Assert.That(
            ValueFormatter.Format(TestExpression.Create("coerce(missing -> :integer)").Evaluate(new RecordValue())),
            Is.EqualTo("{}"));

    [Test]
    public void Coerce_FailedConversion_ReturnsNull()
        => Assert.That(TestExpression.Create("coerce(:integer)").Evaluate("abc"), Is.Null);
    ...
    public void CoerceOrdering_UnrelatedSemanticTargets_ReturnNull()
    {
        Assert.Multiple(() =>
        {
            Assert.That(TestExpression.Create("coerce(:text)").Evaluate(OrderingValue.Less), Is.Null);
            Assert.That(TestExpression.Create("coerce(:boolean)").Evaluate(OrderingValue.Equal), Is.Null);
            Assert.That(TestExpression.Create("coerce(:date)").Evaluate(OrderingValue.Greater), Is.Null);
        });
    }
        ...
        string input)
    {
        var value = input.StartsWith("T(") || input.StartsWith('{')
            ...
            : input;
        Assert.That(
            () => TestExpression.Create(expression).Evaluate(value),
    ...
    public void Coerce_KnownInputShape_InvalidAtBinding_ThrowsBindingError(string expression)
        => Assert.That(() => TestExpression.Create(expression), Throws.TypeOf<BindingException>());
    ...
    public void Coerce_UnavailableTuplePositions_AreIgnored(string expression, string expected)
        => Assert.That(
            ValueFormatter.Format(TestExpression.Create(expression).Evaluate(null)),
            Is.EqualTo(expected));
