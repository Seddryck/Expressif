...
public class RecodeTest
{
    ...
    public void InvalidArity_IsRejected(string expression, Type exceptionType)
        => Assert.That(() => TestExpression.Create(expression), Throws.TypeOf(exceptionType));
    ...
    public void NonDictionaryMapping_IsRejected(string mapping)
        => Assert.That(() => TestExpression.Create($"recode({mapping})").Evaluate(1), Throws.ArgumentException);
    ...
    public void MappingExpressions_ComposeWithSurroundingContext(string source, string expected)
        => Assert.That(ValueFormatter.Format(TestExpression.Create(source).Evaluate(null)), Is.EqualTo(expected));
    ...
    {
        var expression = TestExpression.Create(".code | recode(^.status-codes)");
        Parallel.For(0, 20, i =>
        {
            var input = new RecordValue();
            input.Set("code", "A");
    ...
    {
        var info = ExpressifIntrospection.Functions.Describe().Single(info => info.Name == "recode");
