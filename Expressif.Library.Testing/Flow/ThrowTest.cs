...
public class ThrowTest
{
    ...
    public void Throw_Accepted(object? value, string predicate, string? expected)
        => Assert.That(TestExpression.Create($"throw({predicate}) | upper").Evaluate(value), Is.EqualTo(expected));
    ...
    public void Evaluate_Rejected(string source)
        => Assert.Throws<EvaluationException>(() => TestExpression.Create(source).Evaluate(null));
    ...
    public void Evaluate_RequiresBoolean()
        => Assert.Throws<InvalidCastException>(() => TestExpression.Create("throw(42)").Evaluate(1));
