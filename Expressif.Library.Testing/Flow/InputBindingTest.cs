...
public class InputBindingTest
{
    ...
    public void InputBinding_Named(string source, object? expected)
        => Assert.That(TestExpression.CreateClosed(source).Evaluate(null), Is.EqualTo(NormalizeExpected(expected)));
    ...
    public void Destructuring_RejectsDuplicateNames(string source)
        => Assert.That(() => TestExpression.Create(source), Throws.TypeOf<BindingException>()
    ...
    public void Destructuring_RejectsMalformedLists(string source)
        => Assert.Throws<ExpressifSyntaxException>(() => TestExpression.Create(source));
    ...
    {
        var result = TestExpression.CreateClosed(source).Evaluate(null);
    ...
    public void Named_OuterRecordRemainsStableAcrossNestedArguments()
    {
        ...
        Assert.That(TestExpression.CreateClosed(source).Evaluate(null),
    ...
    {
        var expression = TestExpression.Create("apply(@_ | input :> add(1) | multiply(@input))");
