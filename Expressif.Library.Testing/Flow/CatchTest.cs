...
public class CatchTest
{
    ...
    public void Catch_Recovery(object? value, string expression, string? expected)
        => Assert.That(TestExpression.Create($"catch({expression}) | upper").Evaluate(value), Is.EqualTo(expected));
    ...
    public void Evaluate_MapTerminatesOnlyElementPipeline()
    {
        ...
        Assert.That(TestExpression.Create(source).Evaluate(null), Is.EqualTo(new[] { "BOB", "Alice" }));
    }
    ...
    {
        var count = 0;
        var function = new Catch(() => { count++; return new Expressif.Library.Special.Neutral(); });
    ...
    public void Evaluate_PreservesObjectIdentity()
    {
        ...
        Assert.That(TestExpression.Create("catch(42) | throw()").Evaluate(value), Is.SameAs(value));
    }
    ...
    public void Bind_InvalidArguments(string source)
        => Assert.Catch<ExpressifException>(() => TestExpression.Create(source));

    [Test]
    public void Evaluate_ConcurrentReuse()
    {
        var expression = TestExpression.Create("catch(\"fallback\") | upper");
