...
public class ConditionalTest
{
    ...
    public void Evaluate_AcceptsNull(string source)
        => Assert.That(TestExpression.Create(source).Evaluate(1), Is.Null);

    [TestCase("#null <? #false")]
    [TestCase("#false ?> #null")]
    public void Evaluate_RejectionPreservesInput(string source)
        => Assert.That(TestExpression.Create(source).Evaluate(7), Is.EqualTo(7));

    [TestCase("42 ?> absolute")]
    [TestCase("absolute <? 42")]
    public void Evaluate_RequiresBoolean(string source)
        => Assert.Throws<PredicateEvaluationException>(() => TestExpression.Create(source).Evaluate(-1));
    ...
    public void Parse_RejectsInvalidOperators(string source)
        => Assert.Catch(() => TestExpression.Create(source));
    ...
    {
        var root = (OpenRootExpression)ExpressifBinderFactory.Create().Bind(ExpressionParser.Parse(source));
        var serialized = new ExpressionSerializer().Serialize(root.Expression);
        Assert.That(TestExpression.Create(serialized).Evaluate(-10), Is.EqualTo(TestExpression.Create(source).Evaluate(-10)));
    }

    [Test]
    public void Evaluate_CurrentObjectPreserved()
        => Assert.That(TestExpression.Create("{threshold := 10, value := -12} | .value | absolute <? greater-than(^.threshold)").Evaluate(null), Is.EqualTo(12));

    [Test]
    public void Evaluate_CandidateRunsOnce()
    {
        var calls = 0;
        var expression = new ConditionalForward(value => { calls++; return value; }, _ => false, true);
    ...
    {
        var expression = new ConditionalForward(value => value, _ => throw new InvalidOperationException(), backward);
    ...
    {
        var expression = new ConditionalForward(_ => throw new InvalidOperationException(), _ => true, backward);
        Assert.Throws<InvalidOperationException>(() => expression.Evaluate(1));
    }
    ...
    {
        var expression = new ConditionalForward(_ => throw new InvalidOperationException(), _ => false, false);
        Assert.That(expression.Evaluate(7), Is.EqualTo(7));
    }
    ...
    {
        var expression = TestExpression.Create("absolute <? greater-than(10)");
    ...
    public void Serialize_FieldPipelineRoundTrip()
    {
        ...
        Assert.That(TestExpression.Create(serialized).Evaluate(value), Is.EqualTo(12));
    }

    [Test]
    public void Parse_QuotedOperators()
        => Assert.That(TestExpression.Create("""#true ?> "a ?> b <? c" | neutral""").Evaluate(1), Is.EqualTo("a ?> b <? c"));
}
