...
public class LetTest
{
    ...
    public void UnknownBindings_UseNormalDiagnostic(string source)
        => Assert.That(() => TestExpression.CreateClosed(source).Evaluate(null), Throws.InstanceOf<ExpressifException>());
    ...
    public void InlineInvocations_InheritAndShadow(string source, int[] expected)
        => Assert.That(TestExpression.Create(source).Evaluate(new[] { 1, 2 }), Is.EqualTo(expected));
    ...
    public void NestedInvocations_RestoreOuterValues(string source, int expected)
        => Assert.That(TestExpression.Create(source).Evaluate(1), Is.EqualTo(expected));
    ...
    {
        var expression = TestExpression.Create("let(a := 5) | let(a := 10, b := @a) | @b");
    ...
    public void BindingExpressions_UseTheOriginalRecordContext()
    {
        ...
        var expression = TestExpression.Create("let(total := .quantity | multiply(.unit-price)) | @total");
    ...
    public void NestedRecordBinding_UsesEnclosingValues()
    {
        ...
        var values = (System.Collections.IEnumerable)TestExpression.CreateClosed(source).Evaluate(null)!;
    ...
    {
        var expression = TestExpression.Create("let(original := @_, double := multiply(2)) | add(@double) | add(@original)");
    ...
    public void CurrentInputCapture_UsesThisCallsPipelineInput()
        => Assert.That(TestExpression.Create("add(1) | let(changed := @_) | @changed").Evaluate(10), Is.EqualTo(11));
    ...
    {
        var info = ExpressifIntrospection.Functions.Describe().Single(function => function.Name == "let");
