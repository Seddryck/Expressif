...
public class ControlFlowTest
{
    ...
    public void Evaluate_SkipsLaterBranch(string source)
        => Assert.That(TestExpression.Create(source).Evaluate(1), Is.EqualTo(1));
    ...
    {
        var expression = TestExpression.Create("{threshold := 10, value := -12} | .value | try(absolute => greater-than(^.threshold), _ => 0)");
    ...
    public void Serialize_RoundTrip(string source)
    {
        ...
        Assert.That(TestExpression.Create(serialized).Evaluate(2), Is.EqualTo(TestExpression.Create(source).Evaluate(2)));
    }
    ...
    {
        var expression = TestExpression.Create("try(absolute => greater-than(10), neutral => is-negative, _ => 0)");
    ...
    public void Parse_QuotedDelimiters()
        => Assert.That(TestExpression.Create("""switch(#true => "a, => switch(x)", _ => "b")""").Evaluate(1), Is.EqualTo("a, => switch(x)"));
}
