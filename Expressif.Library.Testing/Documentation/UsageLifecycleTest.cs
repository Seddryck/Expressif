...
public class UsageLifecycleTest
{
    ...
    public void DocumentedMigration_MatchesResolvedDiagnosticAndPreservesResult(UsageLifecycleRule rule, UsageMigrationExample example)
    {
        ...
        var result = TestExpression.Create(example.Expected).Evaluate(null);
        Assert.Multiple(() =>
        {
            ...
            Assert.That(TestExpression.Create(example.Deprecated).Evaluate(null), Is.EqualTo(result));
            Assert.That(TestExpression.Create(example.Replacement).Evaluate(null), Is.EqualTo(result));
        });
    }
    ...
    public void ChunkWhile_FollowingArgumentsRetainCurrentChunkAndCandidate()
    {
        ...
        Assert.That(TestExpression.Create(replacement).Evaluate(null), Is.EqualTo(TestExpression.Create(legacy).Evaluate(null)));
    }
}
