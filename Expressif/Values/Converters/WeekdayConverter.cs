...
namespace Expressif.Values.Converters
{
    ...
    public class WeekdayConverter : TypeConverter
    {
        private IParser<Weekday> Parser { get; }

        public WeekdayConverter()
            : this(new WeekdayCaster()) { }

        public WeekdayConverter(IParser<Weekday> parser)
            => Parser = parser;
        ...
        {
            if (CanConvertTo(destinationType)&& value is Weekday weekday)
