...
Legacy `adjacent(subtract)`, `chunk-while(subtract | less-than(2))`, `map-over(subtract, values)` and `map-with(subtract, values)` remain available. Only reliance on injected arguments is deprecated; the surrounding operators and target callables are not. Prefer `~subtract` for adjacent, chunk-while and map-with, and `subtract~` for map-over. No removal version is assigned.
