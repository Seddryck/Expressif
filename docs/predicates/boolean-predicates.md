...
| [`majority`]({{ '/predicates/boolean/majority/' | relative_url }}) | Returns `true` when strictly more than half of the supplied predicates are satisfied by the input. Returns `false` when no predicates are supplied and stops evaluating as soon as the result is known. |
| [`nand`]({{ '/predicates/boolean/nand/' | relative_url }}) | Returns the negation of the logical conjunction of the Boolean input and a secondary Boolean expression. Evaluates the secondary expression only when the input is `true`. |
| [`nor`]({{ '/predicates/boolean/nor/' | relative_url }}) | Returns the negation of the logical disjunction of the Boolean input and a secondary Boolean expression. Evaluates the secondary expression only when the input is `false`. |
...
| [`satisfies-at-least`]({{ '/predicates/boolean/satisfies-at-least/' | relative_url }}) | Returns `true` when at least the requested number of supplied predicates are satisfied by the input. The count must be non-negative, and evaluation stops as soon as the result is known. |
| [`satisfies-at-most`]({{ '/predicates/boolean/satisfies-at-most/' | relative_url }}) | Returns `true` when at most the requested number of supplied predicates are satisfied by the input. The count must be non-negative, and evaluation stops as soon as the result is known. |
| [`satisfies-exactly`]({{ '/predicates/boolean/satisfies-exactly/' | relative_url }}) | Returns `true` when exactly the requested number of supplied predicates are satisfied by the input. The count must be non-negative, and evaluation stops as soon as the result is known. |
| [`xnor`]({{ '/predicates/boolean/xnor/' | relative_url }}) | Returns the negation of the exclusive disjunction of the Boolean input and a secondary Boolean expression. Always evaluates the secondary expression after the input. |
