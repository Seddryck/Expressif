...
| [`circular-distance`]({{ '/functions/temporal/circular-distance/' | relative_url }}) | Returns the shortest unsigned duration between the current time and a reference time on a 24-hour clock. Returns `null` when either time is `null`. |
