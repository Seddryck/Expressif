...
| [`rename-fields`]({{ '/functions/record/rename-fields/' | relative_url }}) | Transforms selected field names while preserving field values and order. Duplicate resulting names cause an evaluation error. |
| [`select-fields`]({{ '/functions/record/select-fields/' | relative_url }}) | Returns only fields whose names appear in the supplied array, preserving input field order and ignoring unknown names. |
| [`set-private`]({{ '/functions/record/set-private/' | relative_url }}) | Returns a new record by renaming selected public fields by adding a leading underscore, preserving field order and values. Omitting names converts all applicable fields. Existing destination names cause an evaluation error. Unlike public, this function does not remove fields. |
| [`set-public`]({{ '/functions/record/set-public/' | relative_url }}) | Returns a new record by renaming selected private fields by removing one leading underscore, preserving field order and values. Omitting names converts all applicable fields. Existing destination names cause an evaluation error. Unlike public, this function does not remove fields. |
