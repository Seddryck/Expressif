...
| [`extract-matches`]({{ '/functions/text/selection/extract-matches/' | relative_url }}) | Returns complete non-overlapping regular expression matches in source order, including zero-length matches. Returns an empty array for null or empty input or when no match is found. |
