...
| [`tokenize-regex`]({{ '/functions/text/tokenization/tokenize-regex/' | relative_url }}) | Returns segments separated by regular expression matches in source order. Preserves spaces and empty segments without including captured delimiters. Returns an empty array for null or empty input. |
