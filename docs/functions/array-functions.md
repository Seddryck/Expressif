...
| [`zip-strict`]({{ '/functions/array/zip-strict/' | relative_url }}) | Combines corresponding values from equally sized input and parameter arrays into two-element tuples. Throws an invalid-operation error when the arrays have different lengths and returns `null` when either value cannot be evaluated as an array. |
