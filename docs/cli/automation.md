...
A successful `evaluate` writes exactly one formatted value followed by a newline. A successful `run` writes zero or more formatted values. Use `--output json` when a script needs valid JSON, including scalar root values; raw Expressif syntax remains the default. Compact output occupies one line per result; `--output-style pretty` can span several lines. Neither command adds a `Result:` label.
