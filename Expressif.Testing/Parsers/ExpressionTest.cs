...
public class ExpressionTest
{
    ...
    public void Parse_Expression_Valid(string value, int count)
        => Assert.That(Expressif.Parsers.ExpressionParser.Parser.Parse(value).Members.Count, Is.EqualTo(count));
    ...
    public void Parse_ParametrizedExpression_Valid(string value, int count)
        => Assert.That(ExpressionApplicableParser.Parser.Parse(value).Members.Count, Is.EqualTo(count));
}