...
public class OnlyTest
{
    ...
    public void Only_Selection(string value, string predicate, string accumulator, string? expected)
    {
        ...
        Assert.That(Expression.CreateClosed($"{value} | only({predicate}, {accumulator})").Evaluate(null), Is.EqualTo(result));
        Assert.That(Expression.CreateClosed($"{value} | fold(only({predicate}, {accumulator}))").Evaluate(null), Is.EqualTo(result));
    }
    ...
    public void Evaluate_Composition_Valid(string expression, int expected)
        => Assert.That(Expression.Create(expression).Evaluate(new[] { 1, 2, 3, 4 }), Is.EqualTo(expected));

    [Test]
    public void Evaluate_TextAccumulator_PreservesOrder()
        => Assert.That(Expression.CreateClosed("{\"a\", #null, \"b\"} | only(is-not-null, concat(\"-\"))").Evaluate(null), Is.EqualTo("a-b"));

    [Test]
    public void Evaluate_Scan_LeavesStateUnchangedForRejectedItems()
        => Assert.That(Expression.Create("scan(only(is-even, count))").Evaluate(new[] { 1, 2, 3, 4 }), Is.EqualTo(new[] { 0, 1, 1, 2 }));
    ...
    public void Evaluate_LazyInput_EnumeratesOnce()
    {
        ...
        Assert.That(Expression.Create("only(is-even, sum)").Evaluate(Items()), Is.EqualTo(6));
        Assert.That(visits, Is.EqualTo(4));
    }

    [Test]
    public void Evaluate_RecordPredicate_UsesEachItem()
        => Assert.That(Expression.CreateClosed("{{active := #true}, {active := #false}} | only(.active, count)").Evaluate(null), Is.EqualTo(1));

    [Test]
    public void Evaluate_NonBooleanPredicate_Throws()
        => Assert.That(() => Expression.Create("only(neutral, count)").Evaluate(new[] { 1 }), Throws.TypeOf<InvalidCastException>());

    [Test]
    public void Evaluate_AccumulatorFailure_Propagates()
        => Assert.That(() => Expression.Create("only(is-null, sum)").Evaluate(new object?[] { null }), Throws.TypeOf<InvalidCastException>());
    ...
    {
        var wrapper = new OnlyAccumulator(new Expressif.Predicates.Numeric.Even(), new CountAccumulator());
        wrapper.Initialize();
    ...
    {
        var expression = Expression.Create("only(is-even, count)");
    ...
    {
        var info = new AccumulatorIntrospector().Describe().Single(info => info.Name == "only");
    ...
    public void Bind_MissingAccumulator_Throws()
        => Assert.That(() => Expression.Create("only(is-even)"), Throws.TypeOf<MissingRequiredParameterException>());
