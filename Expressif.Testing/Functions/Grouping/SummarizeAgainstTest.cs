...
using SummarizeAgainstFunction = Expressif.Functions.Grouping.SummarizeAgainst;
...
public class SummarizeAgainstTest
{
    ...
    public void Evaluate_PreservesAccumulatorAndCombineFailures()
    {
        ...
        {
            Assert.That(
                () => Expression.Create("#{(\"BE\" => {#null})} | summarize-against(sum, count, first)").Evaluate(null),
    ...
    private sealed class TrackingAccumulator(string name, List<string> events) : IAccumulator
    {
        private int count;
