...
public class TopWithTiesTest
{
    [Conformance]
    public void TopWithTies_Valid_Rows(object? input, string expression, string expected)
    ...
    public void NegativeCount_Throws()
        => Assert.That(() => new TopWithTies(() => -1).Evaluate(new SortTableValue([], [])), Throws.TypeOf<ArgumentOutOfRangeException>());
    ...
    public void InvalidComparer_Throws()
    {
        ...
        Assert.That(() => new TopWithTies(() => 1).Evaluate(table), Throws.TypeOf<InvalidOperationException>());
    }
    ...
    public void ComparerEqualDistinctValues_IncludeEntireBoundary()
    {
        ...
        Assert.That(new TopWithTies(() => 1).Evaluate(table), Is.EqualTo(new[] { "integer", "decimal" }));
    }
}
