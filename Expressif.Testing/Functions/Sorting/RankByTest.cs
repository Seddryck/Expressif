...
public class RankByTest
{
    ...
    public void ExplicitLowering_MatchesSugar(string criterion, string term)
    {
        ...
        var sugar = Expression.Create($"{source} | rank-by({criterion})").Evaluate(null);
        var lowered = Expression.Create($"{source} | map(neutral | pair(sort-key({term}), neutral)) | sort-table | rank").Evaluate(null);
        Assert.That(ValueFormatter.Format(sugar), Is.EqualTo(ValueFormatter.Format(lowered)));
    }

    [Test]
    public void MissingCriteria_Throws()
        => Assert.That(() => Expression.Create("array() | rank-by()").Evaluate(null), Throws.TypeOf<BindingException>());
}
