...
public class ExplodeTest
{
    ...
    public void NonCollectionField_FailsEvaluation(string selected)
        => Assert.That(() => Expression.Create($"{{tags := {selected}}} | explode(.tags)").Evaluate(null),
    ...
    public void NonRecordParent_FailsEvaluation(string source)
        => Assert.That(() => Expression.Create($"{source} | explode(.tags)").Evaluate(null),
            Throws.TypeOf<ArgumentException>());
    ...
    {
        var expression = Expression.Create("explode(.tags)");
        Parallel.For(0, 20, i =>
        {
    ...
    {
        var info = new FunctionIntrospector().Describe().Single(info => info.Name == "explode");
        using (Assert.EnterMultipleScope())
        {
