...
public class ExpandTest
{
    ...
    public void ComputedSelectorDiagnostic_RequiresExplicitLabel()
        => Assert.That(() => Expression.Create("expand(.customer | identity)"),
    ...
    public void NonRecordInput_IsRejected(string input)
        => Assert.That(() => Expression.Create($"{input} | expand(.customer)").Evaluate(null),
            Throws.InstanceOf<ArgumentException>());

    [TestCase("42")]
    [TestCase("#null")]
    public void NonTextLabel_IsRejected(string label)
        => Assert.That(() => Expression.Create($"{{customer := {{name := 1}}}} | expand(.customer, {label})").Evaluate(null),
            Throws.TypeOf<InvalidOperationException>().With.Message.Contains("must return text"));

    [Test]
    public void Label_ReadsIncomingRecord()
        => Assert.That(Expression.Create("{label := \"old\", customer := {name := 1}} | put(label := \"buyer\") | expand(.customer, .label)")
            .Evaluate(null)?.ToString(), Is.EqualTo("{label := \"buyer\", \"buyer.name\" := 1}"));

    [Test]
    public void EmptyLabel_QualifiesFields()
        => Assert.That(Expression.Create("{customer := {name := 1}} | expand(.customer, \"\")").Evaluate(null)?.ToString(),
            Is.EqualTo("{\".name\" := 1}"));
    ...
    public void Evaluation_PreservesValuesOrderAndInput()
    {
        ...
        input.Set("tail", 2);
        var result = (RecordValue)Expression.Create("expand(.customer)").Evaluate(input)!;
    ...
    {
        var expression = Expression.Create("expand(.customer, .label)");
        Parallel.For(0, 30, i =>
        {
            var nested = new RecordValue();
            nested.Set("name", i);
    ...
    {
        var info = new FunctionIntrospector().Describe().Single(info => info.Name == "expand");
        using (Assert.EnterMultipleScope())
        {
