...
public class GenerateTest
{
    ...
    public void Generate_Valid_While_Next_OptionalResult(object? input, string[] parameters, decimal[] expected)
    {
        ...
        var result = parameters.Length == 2
            ? new ExpressionFactory().Instantiate($"generate(while := {parameters[0]}, next := {parameters[1]})", new Context()).Evaluate(seed)
            : new ExpressionFactory().Instantiate($"generate(while := {parameters[0]}, next := {parameters[1]}, result := {parameters[2]})", new Context()).Evaluate(seed);
    ...
    {
        var generate = new ExpressionFactory().Instantiate(
