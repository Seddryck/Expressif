...
namespace Expressif.Testing.Values.Types;

[TestFixture]
[Category("MetadataConsistency")]
public class TypeRegistryTest
{
    private static ITypeRegistry Registry => ExpressifTypeRegistry.Instance;
    ...
    public void GeneratedCatalog_MatchesCanonicalRegistry()
    {
        ...
        Assert.That(generated, Is.EqualTo(Registry.All.Select(type => type.Name).Order()));
    }

    [Test]
    public void Registry_ContainsRequiredValueTypesAndExcludesMetadataConcepts()
    {
        var names = Registry.All.Select(type => type.Name).ToArray();
        Assert.Multiple(() =>
        {
    ...
    public void Registry_ParentMatchesCanonicalHierarchy(string name, string parent)
        => Assert.That(Registry.Resolve(name).Parent, Is.EqualTo(parent));
    ...
    public void Registry_DotNetBindingMatchesCanonicalRuntimeType(string name, Type expected)
        => Assert.That(Registry.Resolve(name).Bindings["dotnet"], Is.EqualTo(expected.FullName));
    ...
    [TestCase("tuple", typeof(Expressif.Values.Tuple))]
    [TestCase("vector", typeof(Vector))]
    ...
    public void RuntimeRegistry_ResolvesImplementationTypeDirectly(string name, Type expected)
        => Assert.That(Registry.ResolveRuntimeType(name), Is.EqualTo(expected));

    [Test]
    public void RuntimeRegistry_FamilyWithoutRuntimeType_ReturnsNull()
        => Assert.That(Registry.ResolveRuntimeType("scalar"), Is.Null);

    [Test]
    public void LiteralExamples_AreAcceptedByParser()
    {
        var examples = Registry.All
    ...
    {
        var parameter = ExpressifBinderFactory.Create().BindParameter(ExpressionParser.Parse($":{name}"));
        Assert.That(parameter, Is.EqualTo(new LiteralParameter(Registry.Resolve(name))));
    }

    [Test]
    public void Binder_UnknownTypeLiteral_ThrowsSpecificError()
        => Assert.That(
            () => ExpressifBinderFactory.Create().BindParameter(ExpressionParser.Parse(":expression")),
    ...
    public void ValueTypeSummary_ComesFromImplementationDocumentation(string name, Type implementationType)
        => Assert.That(Registry.Resolve(name).Summary, Is.EqualTo(implementationType.GetSummary()));

    [Test]
    public void Introspector_UnionsIntrinsicDescriptorsAndImplementedValueTypes()
    {
        var names = new TypeIntrospector(
            typeof(ExpressifTypeRegistry).Assembly,
            typeof(IExpression).Assembly).Describe().Select(descriptor => descriptor.Name).ToArray();
    ...
    {
        var descriptorTypes = typeof(ExpressifTypeRegistry).Assembly.GetTypes()
