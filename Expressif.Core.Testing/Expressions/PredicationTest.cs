...
public class PredicationTest
{
    ...
    {
        Predication predication = TestPredication.Create("lower-case");
    ...
    {
        var predication = TestPredication.Create("within-interval(I[0, 20[)");
    ...
    {
        var predication = TestPredication.Create("!starts-with(\"True\")");
    ...
    {
        var predication = TestPredication.Create("starts-with(\"Nik\") |AND ends-with(\"sla\")");
        var result = predication.Evaluate("Nikola Tesla");
        Assert.That(result, Is.True);
    }
    ...
    {
        var predication = TestPredication.Create("starts-with(\"ola\") |OR ends-with(\"sla\")");
        var result = predication.Evaluate("Nikola Tesla");
        Assert.That(result, Is.True);
    }
    ...
    public void Evaluate_BooleanShorthand_Valid(string code, object value, bool expected)
        => Assert.That(TestPredication.Create(code).Evaluate(value), Is.EqualTo(expected));

    [Test]
    public void Evaluate_CombinationsGroup_Valid()
    {
        var predication = TestPredication.Create("(starts-with(\"Nik\") |AND ends-with(\"sla\")) |OR (starts-with(\"ola\") |AND ends-with(\"Tes\"))");
        var result = predication.Evaluate("Nikola Tesla");
        Assert.That(result, Is.True);

        var withoutGroupsPredication = TestPredication.Create("starts-with(\"Nik\") |AND ends-with(\"sla\") |OR starts-with(\"ola\") |AND ends-with(\"Tes\")");
